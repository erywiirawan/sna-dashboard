...
| 2026-07-22 | Fix: modal "Detail per Class" salah bandingkan YoY saat filter Group+Bulan — data 2026 terpotong bulan tapi 2025 tetap full-year (classes25 tidak ikut dipotong). Backend `agg_sales(class_month=True)` bikin `products.classes_month[grp][bulan][cls]=[rev25,rev26]` hanya untuk group_cache (+~36KB). Frontend `getFilteredSales` blok Group: saat monthActive rekonstruksi classes/classes25/classes26 dari classes_month sesuai bulan terpilih; mergeGroupCaches bawa classes_month per group. Verifikasi KABEL(30KBL) Jan-Jun: 2025 10.16B vs 2026 18.43B → YoY +81.5% (sebelumnya salah -19.1% vs 22.78B full-year) ✅ |
| 2026-07-22 | Fix: "Top 20 Customer" & "Top 20 Item Terlaris" masih campur semua cabang saat filter Group+Cabang aktif — blok Group di getFilteredSales cuma recompute branch bars/monthly/classes, tak pernah filter items & customers (tetap dari group_cache full). Backend bikin cache kompak `group_br_month_items` [kode,nama,rev,qty,rev25,rev26] & `group_br_month_cust` [kode,nama,rev,rev25,rev26] per grp→cabang→bulan (+~5.4MB, JSON jadi ~40MB). Frontend blok Group: saat hasBr, rebuild items+customers+SKU dari cache itu sesuai cabang+bulan terpilih. Verifikasi TBN/KABEL/Jan-Jun: top item & top cust cocok persis ground-truth CSV ✅ |
| 2026-07-22 | Fix: chart "Top 10 Produk" (per-group totals) masih hitung semua cabang saat filter Group+Cabang aktif — blok Group cuma recompute classes/items/customers/total, TAPI `products.values/values25/values26` (yang jadi sumber bar chart) tak pernah di-rebuild (tetap dari group_cache full). Frontend: akumulasi `grpTot[grp]` (rev/rev25/rev26) saat loop `group_br_month_items` per cabang+bulan terpilih, lalu rebuild products.labels/values/values25/values26. Frontend-only (JSON tak berubah). Verifikasi TBN/KABEL/Jan-Jun 2026: chart lama 18.43M (semua cabang) → benar 612.3jt (TBN saja), beda ~30x ✅ |
| 2026-07-22 | Fix: modal "Detail per Class" masih jumlah semua cabang saat filter Group+Cabang aktif — fix Bug 1 rebuild classes dari `classes_month` yang TAK punya dimensi cabang (cuma grup→bulan→class). Backend: tambah `filters.item_class_map` (item→class, ~2380 entri, +~23KB). Frontend blok Group: agregasi class per cabang+bulan dari `group_br_month_items` via item_class_map (clsAll/cls25/cls26), override products.classes/classes25/classes26 — ber-cabang. Verifikasi KABEL/TBN/Jan-Jun: lama 2025 10.16M/2026 18.43M (semua cabang) → benar 2025 876jt/2026 612.3jt (TBN saja, class 30KBLL KABEL SUPREME) ✅ |
